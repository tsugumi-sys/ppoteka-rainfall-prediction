...
class TemperatureInterpolator(InterpolatorInterface):
    ...
    def interpolate(self, ndarray: np.ndarray, observation_point_file_path: str) -> np.ndarray:
        if ndarray.max() > 1 or ndarray.min() < 0:
            raise ValueError(
                f"The scale of the given ndarray is invalid (max: {ndarray.max()}, min: {ndarray.min()}). "
                "The scale should be [0, 1]"
            )
    ...
    def inteHumidityInterplatorrpolate_with_gpu(
        self, ndarray: np.ndarray, observation_point_file_path: str
    ) -> np.ndarray:
        pass
