import json
import sys
import unittest

import numpy
import numpy as np

sys.path.append(".")
from common.config import GridSize, TargetManilaErea  # noqa: E402
from evaluate.src.interpolator.interpolator_interactor import InterpolatorInteractor  # noqa: E402
from evaluate.src.interpolator.pressure_interpolator import PressureInterpolator  # noqa: E402
from evaluate.src.interpolator.rain_interpolator import RainInterpolator  # noqa: E402
from evaluate.src.interpolator.temperature_interpolator import TemperatureInterpolator  # noqa: E402
from evaluate.src.interpolator.wind_interpolator import WindInterpolator  # noqa: E402


class TestInterpolatorInteractor(unittest.TestCase):
    def __init__(self, methodName: str = ...) -> None:
        self.interpolator_interactor = InterpolatorInteractor()
        super().__init__(methodName)

    def test_get_interpolator(self):
        self.assertIsInstance(self.interpolator_interactor.get_interpolator("rain"), RainInterpolator)
        self.assertIsInstance(self.interpolator_interactor.get_interpolator("temperature"), TemperatureInterpolator)
        self.assertIsInstance(self.interpolator_interactor.get_interpolator("wind"), WindInterpolator)
        self.assertIsInstance(self.interpolator_interactor.get_interpolator("u_wind"), WindInterpolator)
        self.assertIsInstance(self.interpolator_interactor.get_interpolator("v_wind"), WindInterpolator)
        self.assertIsInstance(self.interpolator_interactor.get_interpolator("station_pressure"), PressureInterpolator)
        self.assertIsInstance(self.interpolator_interactor.get_interpolator("seaLevel_pressure"), PressureInterpolator)

        with self.assertRaises(ValueError):
            self.interpolator_interactor.get_interpolator("invalidParam")

    def test_interpolate(self):
        weather_param = "rain"

        ob_point_file_path = "./common/meta-data/observation_point.json"
        with open(ob_point_file_path, "r") as f:
            ob_point_data = json.load(f)

        lons_lats = [(item["longitude"], item["latitude"]) for item in ob_point_data.values()]
        target_point = lons_lats[0]

        ob_point_ndarray = np.asarray([0] * len(lons_lats))
        ob_point_ndarray[0] = 1

        grid_array = self.interpolator_interactor.interpolate(weather_param, ob_point_ndarray, ob_point_file_path)
        grid_lons = np.linspace(TargetManilaErea.MIN_LONGITUDE, TargetManilaErea.MAX_LONGITUDE, GridSize.WIDTH)
        grid_lats = np.linspace(TargetManilaErea.MIN_LATITUDE, TargetManilaErea.MAX_LATITUDE, GridSize.HEIGHT)

        target_point_lon_idx, target_point_lat_idx = 0, 0
        for idx, lon in enumerate(grid_lons):
            if lon > target_point[0]:
                target_point_lon_idx = idx
                break

        for idx, lat in enumerate(grid_lats):
            if lat > target_point[1]:
                target_point_lat_idx = GridSize.HEIGHT - idx
                break

        # NOTE: numpy ndarray indexing is array[y, x] if array is two-dimentional.
        grid_data_max_idxs = np.unravel_index(np.argmax(grid_array, axis=None), grid_array.shape)

        self.assertTrue((target_point_lat_idx, target_point_lon_idx) == grid_data_max_idxs)
