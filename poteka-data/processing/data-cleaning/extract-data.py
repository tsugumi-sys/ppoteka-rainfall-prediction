            ...
            for month in monthes:
                ...
                if os.path.exists(path):
                    for date in os.listdir(path):
                        ...
                        #  Check if the observation point has the data of
                        #    year/month/date and csv file of the data
                        if os.path.exists(csv_path) and len(os.listdir(csv_path)) > 0:
                            print("-" * 50)
                            print(csv_path)
                            filename = find_csv_file(csv_path)
                            df_before = pd.read_csv(
                                csv_path + filename, header=None, error_bad_lines=False
                            )
                            ...
                            for col in cols:
                                for col_def in df_before.columns:
                                    if col in str(df_before[col_def].values[0]):
                                        df_before[col] = df_before[col_def].apply(
                                            find_value
                                        )
                            ...
                            if df_before.values.size > 0:
                                ...
                                for col in df_before.columns:
                                    ...
                                    if len(check) == 14:
                                        ...
                                        m = int(str(val)[4:6])
                                        d = int(str(val)[6:8])
                                        df_before["Datetime"] = df_before[col].apply(
                                            str
                                        )
                                        df_before["Datetime"] = df_before[
                                            "Datetime"
                                ...
                                save_path = os.path.join(data_dir_path, "cleaned-data")
